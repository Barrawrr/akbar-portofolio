...
const Masonry = ({ items, ease = "power3.out", duration = 0.6, stagger = 0.05, animateFrom = "bottom", scaleOnHover = true, hoverScale = 0.95, blurToFocus = true, colorShiftOnHover = false }) => {
  ...
  const [containerHeight, setContainerHeight] = useState(0); // ✅ new
  ...
  const grid = useMemo(() => {
    ...
    const mapped = items.map((child) => {
      ...
      const height = child.height / 2; // asumsi lo emang pake height/2
    ...
    // ✅ update container height (kolom tertinggi)
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: containerHeight }}>
      {grid.map((item) => (
        ...
        >
          <Link to={item.link}>
            <div className="relative w-full h-full bg-cover bg-center rounded-[10px] shadow-[0px_10px_50px_-10px_rgba(0,0,0,0.2)] uppercase text-[10px] leading-[10px]" style={{ backgroundImage: `url(${item.img})` }}>
